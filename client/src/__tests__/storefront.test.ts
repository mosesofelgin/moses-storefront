...
describe('MOSES route policy', () => {
  ...
  it('keeps AI Without the Fear as a focused, direct-access lead page with real guide delivery', async () => {
    ...
    const appSource = readFileSync(resolve(process.cwd(), 'client/src/App.tsx'), 'utf8');
    expect(shouldShowVaultGate('/airisks', false)).toBe(false);
    expect(isFocusedRoutePath('/airisks')).toBe(true);
    expect(appSource).toContain('path={"/airisks"}');
    ...
    expect(source).toContain('AI_Without_the_Fear_New_Covenant_Enterprises_d62bbca0.pdf');
    expect(source).toContain('ai-without-the-fear-cover-new_23de119b.jpg');
