  ...
  return (
    <div className="min-h-screen bg-black text-white" style={{ fontFamily: "'DM Mono', monospace" }}>
      <div className="mx-auto max-w-5xl px-4 pt-4">
        <ListenNavigation project="IF I WROTE A MIXTAPE" backHref="/mixtape" backLabel="Project" />
      </div>
      <audio
        ref={audioRef}
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-amber-900/50 bg-black/95 backdrop-blur-md px-4 py-3">
          <div className="mx-auto max-w-5xl flex items-center gap-3 sm:gap-4">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <button onClick={handlePrevious} disabled={currentTrackIndex === 0} aria-label="Previous track" className="flex h-11 w-11 items-center justify-center text-amber-400 hover:text-white disabled:opacity-30 transition-colors">
                <SkipBack className="h-4 w-4" />
              </button>
              <button
                onClick={handlePlayPause}
                aria-label={isPlaying ? 'Pause' : 'Play'}
                className="flex h-11 w-11 items-center justify-center rounded-full bg-amber-700 hover:bg-amber-600 transition-colors"
              >
                {isPlaying ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4 ml-0.5" />}
              </button>
              <button onClick={handleNext} disabled={currentTrackIndex === MIXTAPE_TRACKS.length - 1} aria-label="Next track" className="flex h-11 w-11 items-center justify-center text-amber-400 hover:text-white disabled:opacity-30 transition-colors">
        ...
        <div className="relative z-10 mx-auto max-w-3xl">
          ...
          <div className="flex items-center justify-center gap-4 sm:gap-6 mb-6">
            <button
              ...
              className="flex h-11 w-11 items-center justify-center text-amber-400 hover:text-white disabled:opacity-30 transition-colors"
            >
          ...
          <div className="flex items-center gap-3 justify-center mb-8">
            <button onClick={toggleMute} aria-label={isMuted || volume === 0 ? 'Unmute' : 'Mute'} className="flex h-11 w-11 items-center justify-center text-stone-500 hover:text-amber-400 transition-colors">
              {isMuted || volume === 0
          ...
          <div className="flex justify-center">
            <DownloadButton
              endpoint={MIXTAPE_META.downloadEndpoint}
              filename={MIXTAPE_META.zipFilename}
              label="Download Full Project"
              variant="outline"
              size="md"
              className="rounded-lg border-amber-700 font-bebas text-base tracking-wide text-amber-200 hover:border-amber-400 hover:text-white"
            />
          </div>
        </div>
      </section>
                ...
                >
                  ...
                  <div onClick={(event) => event.stopPropagation()}>
                    <DownloadButton
                      ...
                      size="sm"
                      iconOnly
                      className="border-transparent bg-transparent p-2 text-stone-600 hover:border-amber-900/50 hover:bg-amber-900/20 hover:text-amber-400"
                    />
                  </div>
                </div>
              );
